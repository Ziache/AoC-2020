...
def weight(cline, color):
    # returns the weight associated with a successor

    for i in range(len(cline)):
        # if the color is recognized, we return the following weight
        if str(cline[i]) == color:
            return cline[i + 1]
...
def computeoneinclusion(incdict, clines, color):
    ...
    # the number of other bags included is 0
    if l == 1:
        incdict[color] = 0
        return incdict
    # else
        ...
        if gor(undefined):
            ...
            for c in succ:
                ...
                if incdict[c] == -1:
                    incdict = computeoneinclusion(incdict, clines, c)
                # else, we do nothing
                else:
                    pass

        # at that point, all the successors' values have been computed.
        # the total number of bags is the number of successors
        #                           + the number of bags in the successors
        valsucc = [(incdict[col] + 1) * weight(cline, col) for col in succ]
        incdict[color] = bigsum(valsucc)
        return incdict


def computeinclusion(clines):
    ...
    for col in colors:
        ...
        if incdict[col] == -1:
            incdict = computeoneinclusion(incdict, clines, col)
        # else, we do nothing
        else:
            pass
...
with open("input.txt", "r") as f:
    ...
    incdict = computeinclusion(clines)

    print(incdict['shiny gold'])
